...
#   exit 2 — gate could not run (no SD_BIN / harness not staged)
#
# Env:
...
#                      until sdd-testwin authors it, this exits 2 gracefully)
...
if (-not (Test-Path $harness)) {
    Say "WARN: harness dir not found: $harness (set SD_HARNESS_DIR). Gate cannot run."
    exit 2
}
$wrapperPath = Join-Path $harness $wrapper
if (-not (Test-Path $wrapperPath)) {
    Say "WARN: gate wrapper not present: $wrapperPath (sdd-testwin authors it). Gate cannot run."
    exit 2
}
