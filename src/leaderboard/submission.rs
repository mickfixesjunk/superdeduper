...
// Codex-review item 3 (v0.3.27 follow-on, 2026-06-02): the disk-queue +
// archive + review storage moved to `crate::leaderboard::submission_store`.
// Re-exported here so every existing call site that wrote
// `submission::queue_dir(...)` / `submission::archive_attempt(...)` /
// `submission::flag_for_review(...)` / `submission::enqueue(...)` etc
// continues to resolve unchanged via the pub-use shim.
pub use super::submission_store::{
    archive_attempt, archive_dir, enqueue, flag_for_review, queue_dir, review_dir,
};

// Codex-review item 3 (v0.3.27 follow-on, 2026-06-02): everything from
// archive_attempt through QueuedSubmission moved to
// crate::leaderboard::submission_store. The pub-use shim above this
// block keeps engine call sites resolving unchanged.

// Codex-review item 3 (v0.3.27 2026-06-02): the GUI-state slots
// (PENDING / LAST_OUTCOME / PENDING_SUBMISSION_ID + their getters/
// setters) moved to `crate::leaderboard::gui_submission_state`.
// Re-exported here so every existing call site that wrote
// `submission::store_pending(...)` / `::peek_pending(...)` etc
// continues to resolve unchanged.
pub use super::gui_submission_state::{
    clear_last_outcome, clear_pending_submission_id, peek_last_outcome, peek_pending,
    peek_pending_submission_id, store_last_outcome, store_pending,
    store_pending_submission_id, take_pending, update_last_outcome_ranks,
};

#[cfg(test)]
mod tests {
    use super::*;
    use super::super::hmac_signer;
    use super::super::submission_store::{now_iso8601, outcome_kind_tag, SerializableOutcome};

    /// Test-only inverse of `now_iso8601`: render a unix timestamp as
    /// RFC 3339 UTC. Used by anchor tests; production code paths go
    /// via `crate::time::now_iso8601`.
