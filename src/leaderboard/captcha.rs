...
/// Open `url` in the user's default browser. Single cross-platform
/// helper lives in `crate::platform::open_url`; this is a thin
/// wrapper that maps the PlatformError into `CaptchaError::BrowserOpenFailed`
/// so existing call sites don't have to know about the platform-
/// error shape. Per #74 — was previously duplicated against
/// `oauth.rs::open_browser_windows` with identical ShellExecuteW
/// + xdg-open / open dispatch logic.
fn open_browser(url: &str) -> Result<(), CaptchaError> {
    crate::platform::open_url(url).map_err(|e| CaptchaError::BrowserOpenFailed(format!("{e:?}")))
}
