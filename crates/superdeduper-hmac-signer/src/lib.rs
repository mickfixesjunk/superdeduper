...
//!
//! Phase 0 (2026-05-31): extracted into a leaf crate so the future
//! bench-real / bench-stub crates can depend on it without depending
//! on the entire engine binary. Engine's `leaderboard::hmac_signer`
//! is now a thin re-export shim around this crate so existing callsites
//! (engine + GUI + leaderboard internals) compile unchanged.
...
/// 32-byte HMAC key. Matches the engine's `InstallKey = [u8; 32]` alias.
/// Kept as a raw byte slice ref in the public surface so this crate stays
/// dependency-free (no need to import an `InstallKey` type from engine).
pub type Key = [u8; 32];

/// Sign `body` with the install's HMAC key. Returns the hex-encoded
/// signature for the `X-Sd-Signature` header.
pub fn sign(install_key: &Key, body: &[u8]) -> String {
...
pub fn sign_canonical(install_key: &Key, canonical: &str) -> String {
