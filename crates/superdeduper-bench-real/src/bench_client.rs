...
//!
//! Phase 2-B 2026-06-01: file moved here from
//! `src/leaderboard/bench_client.rs`. The file-level cfg gate on
//! the engine-side version is removed -- bench-real crate is feature-
//! gated at the engine's Cargo.toml level instead. Engine retains a
//! thin re-export shim at the old path so existing call sites resolve
//! unchanged.
//!
//! Original module-level rationale (kept for reference):
//! The public engine's `--bench-me` does ONLY: download the corpus,
//! run the real product dedupe, ANSWER the server's challenge by
//! hashing the downloaded bytes at server-issued offsets, submit.
//! There is NO corpus generation, NO seed, NO Merkle tree on the
//! client -- the server holds the private seed + groundtruth and
//! verifies directly. This module SURVIVES the generator strip. It
//! depends only on BLAKE3 + std fs; no ChaCha20, no tree-build, no
//! plan/manifest.
...
// Phase 2-B 2026-06-01: wire-shape types canonicalized in
// bench-iface (see iface-types-first sequence commits). Re-exported
// here so existing code that wrote `bench_client::ChallengePosition`,
// `bench_client::ChallengeAnswer`, or `bench_client::CanonicalBench`
// continues to resolve via this module path.
...
mod tests {
    ...
    // Phase 2-B 2026-06-01: to_canonical_bench_assembles_submission_block
    // is disabled inside bench-real because it reaches into engine's
    // submission::build_payload, which would create a circular dep
    // (bench-real -> engine -> bench-real). The test will be moved to
    // an engine-side integration test once Phase 2-B's submission move
    // + call-site rewrites complete. Tracked in HANDOVER.
    #[cfg(any())]
    #[test]
