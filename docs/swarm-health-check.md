...
- **dumbo** is in the `giga-superdeduper` tmux session (window 16; was moved here from a separate Claude Code session at some point after this doc was originally drafted) but is explicitly listed in `EXCLUDED_WINDOWS` for cleanroom isolation per [[feedback_dumbo_cleanroom_isolation]]. Reading dumbo's pane wouldn't itself leak, but flagging dumbo `WEDGED` would invite a design-boss nudge — and nudging dumbo injects design's text into a cleanroom session, exactly the leak the isolation policy forbids. Cheaper to skip the sweep entirely.
